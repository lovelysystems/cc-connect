...
- **claudecode**: AskUserQuestion always reaches its interactive prompt regardless of permission mode. Previously `dontAsk` auto-denied it and `bypassPermissions` auto-allowed it with an empty answer, so the question never rendered and the agent fell back to prose.
