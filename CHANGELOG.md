...
- **core**: stop duplicating pre-boundary text on streaming-card replies that have no live message preview. When tool and thinking progress messages are hidden (`tool_messages = false` / `thinking_messages = false`, in any non-quiet mode) and the turn streams as a card while `canPreview()` is false — the platform is not a `MessageUpdater` (e.g. DingTalk), or stream preview is disabled/degraded — the pre-tool / pre-thinking text segment was flushed as a standalone plain message even though the streaming card already rendered it, so the lead-in appeared twice. The standalone flush at both the thinking and tool boundaries is now skipped while a healthy streaming card owns the turn, and still runs as a fallback when the card has failed. Platforms with an active live preview (e.g. Slack via `UpdateMessage`) already took the preview path and were unaffected.
- **claudecode / core**: resume validation now uses the session's real working directory (#599 follow-up). `ValidateSessionID` derived the transcript's per-project directory solely from the configured `work_dir`, but Claude Code names that directory after the agent process's actual cwd. When a custom `agent.options.cmd` runs the agent in a different directory (e.g. a per-session workspace wrapper), the transcript lived under a project dir the validator never constructed, so after a daemon restart the stored session failed validation and started fresh — losing all conversation context. cc-connect now captures the `cwd` the CLI reports in its `system`/`init` event, persists it on the session as `agent_cwd`, and validates the transcript against that directory when the cwd is within the agent's `work_dir` (a per-session subdir), falling back to `work_dir` otherwise or for records with no captured cwd (backward compatible, no migration). Cross-project isolation (#599) is preserved: a cwd outside `work_dir` — which cannot be told apart from another project's cwd leaked onto a shared session row — is not trusted and falls back to `work_dir`. Downstream-fork fix; to be ported upstream.
